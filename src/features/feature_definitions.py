...
def test_fuiture_build(df,tag):
    datetime_features_fix(df)
    create_distance(df)
...
if __name__ == '__main__':
    ...
    test_fuiture_build(data.head(10), 'train')
    
    
    
#EXPLENATION:
#Read the file from data/raw folder 
#pass to the function test_fuiture_build 
# (this function will create the new features. we will be using this function to create extra features in real time
# so whenever customer gives input to model, our model will take those inputs create new feature and then model will
# do the prediction. Note that the model is trained on newly built features)